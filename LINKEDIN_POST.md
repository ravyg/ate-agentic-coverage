...
The labels were generated with a language model against a neutral instrument, in one consistent pass across all 18,796 tasks, then checked by hand. Three independent annotators each scored the same 200-task audit sample, blind to the model's answers. The model's ranking reaches a Spearman correlation of 0.884 against the human panel, and stays between 0.87 and 0.88 whichever annotators you use, so no single rater is carrying it.
