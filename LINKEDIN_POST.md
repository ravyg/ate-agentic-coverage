...
It scores all 18,796 O*NET work tasks, across every one of the 23 SOC major groups, on a single 0 to 1 measure of agentic coverage: how much of that task an autonomous agent could complete end to end, with no human stepping in. Every task also carries a short written rationale for its score.

Most AI exposure research asks whether a job could be automated. That blurs a lot. A single job title hides dozens of very different tasks, and exposure does not arrive at the job level, it arrives task by task. This dataset lets you work one level down. 43.7% of tasks score under 0.2, work that still needs a person throughout. 18.8% score 0.8 or higher, work an agent can already carry almost alone. The middle is a genuine spread rather than a cliff, which is itself the useful finding.

The labels were generated with a language model against a neutral instrument, applied in one consistent pass across all 18,796 tasks. Then they were checked by hand. Three independent annotators each scored the same 200-task audit sample, blind to the model's answers. The model's ranking reaches a Spearman correlation of 0.884 against the human panel, which is 99.9% of the ceiling set by how much the three annotators agree with each other. In other words the ordering is about as good as it can get.

One honest caveat, and it is in the repo rather than buried. The model runs conservative in absolute terms, scoring tasks 0.147 lower on average than humans do. Rankings are trustworthy as released. If you need calibrated absolute numbers, apply the correction we describe. The full method, the agreement statistics, and the script that regenerates every published number are all in the repo so you can judge the quality yourself.
