...
HELPERS = """
    ...
    // Stable session id derived from identity, so a refresh / different device
    // maps to the same person and resumes (and re-rating upserts, not dupes).
    function sidFor(name, email) {
      var key = (String(name) + '|' + String(email)).toLowerCase().replace(/\\s+/g, ' ').trim();
      var h = 5381; for (var j = 0; j < key.length; j++) { h = ((h << 5) + h + key.charCodeAt(j)) >>> 0; }
      var slug = key.replace(/[^a-z0-9]+/g, '').slice(0, 12);
...
<div id="gate" class="gate">
  ...
  <div class="credit-note">
    ...
    <b>in progress</b>. Every verified contributor is <b>named in the Acknowledgements</b>
    of the dataset and its paper. Add your name (email optional, for credit) to begin.
  </div>
  <input id="g-name" type="text" placeholder="Your name (or nickname)">
  <input id="g-email" type="email" placeholder="Email (optional, for credit)">
...
__HELPERS__
    ...
    function enterTool() {
      ...
      sessionId = sidFor(annName, annEmail);
      saveIdent();
